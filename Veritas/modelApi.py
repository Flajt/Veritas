...
But I can't deciver the preprocessing.
    ...
    def translate(self, text: list):
        """Translates a given list of text into a sparql querry"""
        batches: Generator = self._make_batches(text)
        self.translator.generate([self.m.f],sample=batches)

    def _make_batches(self, lines, max_tokens=None, max_sentences=None,task=None, max_positions=None):
        """Builds batches from lines of text. Credit: Facebooks fairseq module"""
        tokens = [
    ...
    def _main(self,args):
        ...
        task = self.tasks.setup_task(args)
        ...
        for inputs in self._buffered_read(args.buffer_size):
            indices = []
            results = []
            for batch, batch_indices in self._make_batches(inputs, args, task, max_positions):
